...
(deftask dev []
  ...
  (comp
    ...
    (reload :on-jsload 'quatrefoil.main/on-jsload!
            :cljs-asset-path ".")
...
(deftask rsync []
  (with-pre-wrap fileset
    (sh "rsync" "-r" "target/" "quamolit.org:repo/Quamolit/quatrefoil" "--exclude" "main.out" "--delete")
    fileset))
...
(deftask watch-test []
  ...
  (comp
    (watch)
    (test :namespaces '#{quatrefoil.test})))
